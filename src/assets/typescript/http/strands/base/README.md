...
The main entrypoint to your app is defined in `main.ts`. It creates an Express server with the Strands Agent SDK
running within, exposing the `/ping` and `/invocations` endpoints required by AgentCore Runtime.
...
If installation was successful, dependencies are already installed in `node_modules/`.
